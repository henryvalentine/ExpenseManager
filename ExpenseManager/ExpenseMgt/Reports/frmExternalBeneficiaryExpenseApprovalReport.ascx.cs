...
{
    public partial class FrmApprovalReport1 : UserControl
    {
        #region Methods
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                dgApprovedTransactions.DataSource=new List<ExpenseApproval>();
        ...
        private bool GetTransactionsByDate()
        {
            ...
            try
            {
                    ...
                    var approvedTransactionsByDate = new List<ExpenseApproval>();
                    ...
                    if (!string.IsNullOrEmpty(txtStart.Text.Trim()) && !string.IsNullOrEmpty(txtEndDate.Text.Trim()))
                    {
                        ...
                        approvedTransactionsByDate = ServiceProvider.Instance().GetExpenseApprovalServices().GetExpenseApprovalsDateRange(startDate, endDate);
                    }
                    ...
                    else
                    {
                        ...
                        {
                            approvedTransactionsByDate = ServiceProvider.Instance().GetExpenseApprovalServices().GetExpenseApprovalsDate(DateMap.ReverseToServerDate(txtEndDate.Text.Trim()));
                        }

                        if (string.IsNullOrEmpty(txtEndDate.Text.Trim()) && !string.IsNullOrEmpty(txtStart.Text.Trim()))
                        {
                            approvedTransactionsByDate =
                                ServiceProvider.Instance().GetExpenseApprovalServices().GetExpenseApprovalsDate(DateMap.ReverseToServerDate(txtStart.Text.Trim()));
                        }
        ...
        private bool LoadAllTransactions()
        {
            try
            {
                ...
                var allApprovedTransactions = ServiceProvider.Instance().GetExpenseApprovalServices().GetAllExpenseApprovals();
