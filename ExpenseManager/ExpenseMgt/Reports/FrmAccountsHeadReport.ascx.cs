...
{
    public partial class FrmAccountsHeadReport : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                dgCostItems.DataSource = new List<TransactionItem>();
        ...
        protected void BtnDateFilterClick(object sender, EventArgs e)
        {
            ...
            GetAccountsHeadsCostsByDateRange();
        }
        private void LoadAccountsHeads()
        {
            try
            {
                var items = ServiceProvider.Instance().GetAccountsHeadServices().GetOrderedAccountsHeads();
                if (!items.Any())
                {
                    ddlAccountsHeads.DataSource = new List<ExpenseItem>();
                ...
                ddlAccountsHeads.DataSource = items;
                ddlAccountsHeads.DataValueField = "AccountsHeadId";
                ddlAccountsHeads.DataTextField = "Title";
                ddlAccountsHeads.DataBind();
                ddlAccountsHeads.Items.Insert(0, new ListItem("--- Select Account Head ---", "0"));
                ddlAccountsHeads.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
        ...
        protected void FirstLink(object sender, EventArgs e)
        {
            try
            {
                ...
                FillRepeater<AccountsHead>(dgCostItems, "_accountsHeads", Navigation.Sorting, Limit, LoadMethod);
            }
            catch (Exception ex)
            {
                ErrorManager.LogApplicationError(ex.StackTrace, ex.Source, ex.Message);
        ...
        protected void LbtnNextClick(object sender, EventArgs e)
        {
            try
            {
                FillRepeater<AccountsHead>(dgCostItems, "_accountsHeads", Navigation.Next, Limit, LoadMethod);
            }
            catch (Exception ex)
            {
                ErrorManager.LogApplicationError(ex.StackTrace, ex.Source, ex.Message);
        ...
        protected void LbtnFirstClick(object sender, EventArgs e)
        {
            try
            {
                FillRepeater<AccountsHead>(dgCostItems, "_accountsHeads", Navigation.First, Limit, LoadMethod);
            }
            catch (Exception ex)
            {
                ErrorManager.LogApplicationError(ex.StackTrace, ex.Source, ex.Message);
        ...
        protected void LbtnLastClick(object sender, EventArgs e)
        {
            try
            {
                FillRepeater<AccountsHead>(dgCostItems, "_accountsHeads", Navigation.Last, Limit, LoadMethod);
            }
            catch (Exception ex)
            {
                ErrorManager.LogApplicationError(ex.StackTrace, ex.Source, ex.Message);
        ...
        protected void LbtnPrevClick(object sender, EventArgs e)
        {
            try
            {
                FillRepeater<AccountsHead>(dgCostItems, "_accountsHeads", Navigation.Previous, Limit, LoadMethod);
            }
            catch (Exception ex)
            {
                ErrorManager.LogApplicationError(ex.StackTrace, ex.Source, ex.Message);
        ...
        protected void OnLimitChange(object sender, EventArgs e)
        {
            ...
            {
                Limit = int.Parse(ddlLimit.SelectedValue);
                FillRepeater<AccountsHead>(dgCostItems, "_accountsHeads", Navigation.None, Limit, LoadMethod);
            }
            catch (Exception ex)
            {
                ErrorManager.LogApplicationError(ex.StackTrace, ex.Source, ex.Message);
        ...
        {
            if (!GetAccountsHeadsCostsByDateRange())
            {
                return false;
            }
            return true;
